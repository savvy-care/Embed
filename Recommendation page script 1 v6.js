...
function HandleRecReason(product, num,xx){
	let result = rec_reason_dict[String(product)+"-"+String(fo_r)];
	if (result === undefined){
		return;
	} else {
		xx.getElementsByClassName("rec-reason")[0].innerText = result;
	}
}
...
function assessRecReason(product,xx){
	...
	if (product =="Magnesium"){
		if (mgr > 0) {
			HandleRecReason(product, mgr,xx);
		}
	}
	if (product =="Fish Oil"){
		if (fo_r > 0) {
			HandleRecReason(product, fo_r,xx);
		}
	}
	if (product =="Vitamin B12" || product=="Vitamin B Complex"){
		if (b12r > 0) {
			HandleRecReason(product, b12r,xx);
		}
	}
	if (product =="Vitamin C"){
		if (vcr > 0) {
			HandleRecReason(product, vcr,xx);
		}
	}
	if (product =="Turmeric"){
		if (tr > 0) {
			HandleRecReason(product, tr,xx);
		}
	}
}
